...
sqlExport() {
    ...
    ssh $USER@$HOST -p $PORT -i /tmp/$tmpKeyName "cd ~;mysqldump '$dbName' -u'$dbUser' -p'$userPass' -v --single-transaction | gzip -9" > auto_$dbName.sql.gz && gzip -d auto_$dbName.sql.gz
