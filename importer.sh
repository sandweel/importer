...
sshKeygen() {
    ...
    if [[ -z $PASSWORD ]];then
        sshExec="ssh -oStrictHostKeyChecking=no $USER@$HOST -i /tmp/$tmpKeyName -p $PORT"
    else
        sshExec="sshpass -p$PASSWORD ssh -oStrictHostKeyChecking=no $USER@$HOST -p $PORT"
    fi
}
...
sqlExport() {
    if [[ $cms == "m1" ]];then
        ...
        sqlDataInfo=`$sshExec "cat $configPath" | grep -m $grepKey -A 6 "'default' =>" | grep "host\|username\|password\|dbname" | sort | uniq | awk -F"'" {'print $2,$4'}`
        dbHost=`echo "$sqlDataInfo" | grep "host" | awk {'print $2'}`
        dbName=`echo "$sqlDataInfo" | grep "dbname" | awk {'print $2'}`
        dbUser=`echo "$sqlDataInfo" | grep "username" |  awk {'print $2'}`
        userPass=`echo "$sqlDataInfo" | grep "password" |  awk {'print $2'}`
